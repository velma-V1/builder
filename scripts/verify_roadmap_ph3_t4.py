...
def verify_migration_and_schema() -> VerificationResult:
    ...
    return VerificationResult("Audit migration: append-only + UNIQUE(op_key, record_kind)", ok, detail)


def verify_migration_sha_pinning() -> VerificationResult:
    ok, detail = _contains("src/factory/audit/writer.py", "0001_audit_chain.sql", "_EXPECTED_MIGRATION_HASHES")
...
def verify_models() -> VerificationResult:
    ...
    return VerificationResult("Audit models (RecordKind, AuditRecord, AuditEvent, BreakClass)", ok, detail)
...
def verify_tests_exist() -> VerificationResult:
    ...
    return VerificationResult(
        "Audit test files (unit/security/failure-path)", len(present) == len(files),
...
def verify_tests_pass_with_coverage() -> VerificationResult:
    result = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/audit", "-q",
         "--cov=src/factory/audit", "--cov-branch", "--cov-fail-under=95"],
        capture_output=True, text=True,
    )
    summary = (result.stdout.strip().split("\n") or ["?"])[-1]
    return VerificationResult(
        "Audit tests pass with >=95% branch coverage", result.returncode == 0,
...
def verify_ruff() -> VerificationResult:
    ...
    return VerificationResult("Ruff clean (audit src + tests)", result.returncode == 0, f"exit {result.returncode}")
...
def verify_mypy() -> VerificationResult:
    ...
    return VerificationResult("mypy --strict clean (audit)", result.returncode == 0, f"exit {result.returncode}")
...
def main() -> int:
    ...
    if passed == total:
        print("RPH3-T4 gate: PASS (audit foundation). NOT PROM-RPH3; NOT implementation of other tasks.\n")
        return 0
