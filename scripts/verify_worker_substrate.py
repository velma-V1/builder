...
def verify_module_files() -> VerificationResult:
    ...
    return VerificationResult(
        "Worker Engine module files exist",
...
def verify_task_executor() -> VerificationResult:
    ...
    missing = [n for n in needed if n not in content]
    return VerificationResult(
        "TaskExecutor + streaming seam present", not missing, f"missing {missing}"
    )
...
def verify_lease_coordinator() -> VerificationResult:
    ...
    missing = [n for n in needed if n not in content]
    return VerificationResult(
        "LeaseCoordinator wraps LeaseManager", not missing, f"missing {missing}"
    )
...
def verify_writer_not_exported() -> VerificationResult:
    ...
    return VerificationResult(
        "SEC-PH3-01: writer NOT exported from workers package",
        passed,
...
def verify_recovery_module() -> VerificationResult:
    ...
    missing = [n for n in needed if n not in content]
    return VerificationResult(
        "Recovery + reconciliation application present", not missing, f"missing {missing}"
    )
...
def verify_success_is_verifying_not_complete() -> VerificationResult:
    ...
    return VerificationResult(
        "Worker success → VERIFYING (no self-certify COMPLETE)",
        passed,
...
def verify_sec_single_writer_test() -> VerificationResult:
    ...
    return VerificationResult(
        "SEC-PH3-01 confinement test exists", passed, "security/test_single_writer_confinement.py"
    )
...
def verify_sec_fencing_test() -> VerificationResult:
    ...
    return VerificationResult(
        "SEC-PH3-02 epoch fencing test exists",
        passed,
        "failure_paths/test_lease_epoch_staleness.py",
    )
...
def verify_sec_output_bounds_test() -> VerificationResult:
    ...
    return VerificationResult(
        "SEC-PH3-03 output-bounds test exists", passed, "security/test_output_bounds.py"
    )


def verify_no_blind_resume_test() -> VerificationResult:
    passed = _test_file_mentions(
        "unit/test_recovery.py", ["BLOCKED", "no_resume"]
    ) or _test_file_mentions("unit/test_recovery.py", ["BLOCKED"])
    return VerificationResult(
        "SEC-PH3-04 no-blind-resume (RUNNING→BLOCKED) test exists", passed, "R3"
    )
...
def verify_failure_path_tests() -> VerificationResult:
    ...
    needed = {
        "test_worker_crash_detection.py",
        "test_execution_failures.py",
        "test_recovery_failures.py",
        "test_lease_epoch_staleness.py",
        "test_state_integration_failures.py",
    }
    found = {f.name for f in fp.glob("*.py")}
    return VerificationResult(
        "Failure-path suites present", needed.issubset(found), f"missing {sorted(needed - found)}"
    )
...
def verify_integration_three_cycle() -> VerificationResult:
    ...
    return VerificationResult(
        "Integration 3-cycle recovery test exists", passed, "state consistency across restarts"
    )
...
def verify_ph2_regression() -> VerificationResult:
    ...
    summary = result.stdout.strip().split("\n")[-1] if result.stdout else ""
    return VerificationResult(
        "PH-2 orchestrator regression (93 tests)", result.returncode == 0, summary
    )
...
def verify_ruff() -> VerificationResult:
    ...
    return VerificationResult(
        "Ruff linting passes", result.returncode == 0, f"exit {result.returncode}"
    )
...
def verify_mypy() -> VerificationResult:
    ...
    return VerificationResult(
        "mypy --strict passes", result.returncode == 0, f"exit {result.returncode}"
    )
