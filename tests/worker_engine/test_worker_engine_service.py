...
def current_state(
    reader: SQLiteOrchestratorStateReader, task_id: str
) -> TaskState:
...
def test_direct_read_only_task_completes_with_no_sandbox_or_staging(
    service: WorkerEngineService, orchestrator_writer: _OrchestratorStateWriter,
...
def test_staged_write_task_uses_a_temp_staging_dir_not_a_git_worktree(
    ...
    assert run.sandbox_path is None  # never a git worktree for STAGED_WRITE
...
def test_policy_forces_sandbox_even_for_a_read_only_request(
    ...
    task_id = submit_task(
        orchestrator_writer, description="command: install_dependency\nAdd a new package.",
    )
    summary = service.claim_and_run(task_id)
    assert summary is not None
    assert summary.selected_mode is ExecutionMode.SANDBOXED_EXECUTION
...
def test_duplicate_claim_race_only_one_thread_wins(
    ...
    git: GitManager, workspace_manager: WorkspaceManager, repo: Path,
    ...
    services = [
        WorkerEngineService(
            orchestrator_writer=orchestrator_writer, orchestrator_reader=orchestrator_reader,
            run_writer=run_writer, run_reader=run_reader, git=git,
            workspace_manager=workspace_manager, repo_root=repo, model_router=successful_router,
        )
        for _ in range(8)
    ]
...
def test_crash_reconciliation_never_blindly_resumes_a_running_task(
    orchestrator_writer: _OrchestratorStateWriter,
    orchestrator_reader: SQLiteOrchestratorStateReader,
    run_writer: _WorkerRunWriter, run_reader: SQLiteWorkerRunReader,
    git: GitManager, workspace_manager: WorkspaceManager, repo: Path,
    successful_router: FakeModelRouter,
) -> None:
    task_id = submit_task(orchestrator_writer, description="Explain what this repo does.")
    ...
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.QUEUED,
        new_state=TaskState.PLANNING, cause="dispatch", actor="prior-process",
    )
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.PLANNING,
        new_state=TaskState.RUNNING, cause="worker_ready", actor="prior-process",
    )
...
def test_retry_exhaustion_eventually_finalizes_to_failed(
    ...
    service = WorkerEngineService(
        orchestrator_writer=orchestrator_writer, orchestrator_reader=orchestrator_reader,
        run_writer=run_writer, run_reader=run_reader, git=git,
        workspace_manager=workspace_manager, repo_root=repo, model_router=failing_router,
...
def test_retry_blocked_tasks_never_touches_a_task_blocked_for_an_unrelated_reason(
    ...
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.QUEUED,
        new_state=TaskState.PLANNING, cause="dispatch", actor="x",
    )
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.PLANNING,
        new_state=TaskState.RUNNING, cause="worker_ready", actor="x",
    )
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.RUNNING,
        new_state=TaskState.BLOCKED, cause="manual_review_requested", actor="operator",
    )
...
def test_agent_zero_unavailable_falls_back_to_builder_native_explicitly(
    ...
    unreachable_client = AgentZeroProcessClient(
        base_url="http://127.0.0.1:1", ollama_base_url="http://127.0.0.1:11434",
...
def test_cancellation_before_execution_starts_is_honored(
    ...
    orchestrator_writer.apply_transition(
        task_id=task_id, expected_current_state=TaskState.QUEUED,
        new_state=TaskState.PLANNING, cause="pre_cancel_planning", actor="op",
    )
    ...
    service = WorkerEngineService(
        orchestrator_writer=orchestrator_writer, orchestrator_reader=orchestrator_reader,
        run_writer=run_writer, run_reader=run_reader, git=git,
        workspace_manager=workspace_manager, repo_root=repo,
