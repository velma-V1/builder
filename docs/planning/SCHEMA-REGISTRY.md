...
| security-spine (approval/permission/tool + `*_intents` + `intervention_journal`) | ordered per-domain migrations under `migrations/security/`: `0001_security_spine.sql` (approval, RPH3-T3), `0002_permission.sql` (permission, RPH3-T2), `0003_tools.sql` (tool registry/declarations/quarantine, RPH3-T5); watchdog-journal (WIR) as a later `00NN_*` | PH-3 | PH-3-owned store, separate from runtime DB (DEP-RPH3 §2/§3/§3.1, ODI-RPH3-01, operator-adopted ordered per-domain model); each SHA-pinned + single-transaction; per-domain sole writer incl. operation-intent tables (XSC-RPH3); cumulative end-state inventory unchanged; R1 preserved |
