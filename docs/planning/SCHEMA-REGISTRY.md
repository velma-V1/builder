...
| security-spine (approval/permission/tool + `*_intents` + `watchdog_intervention_journal`) | ordered per-domain migrations under `migrations/security/`: `0001_security_spine.sql` (approval, RPH3-T3), `0002_permission.sql` (permission, RPH3-T2), `0003_tools.sql` (tool registry/declarations/quarantine, RPH3-T5), `0004_watchdog.sql` (internal WIR journal, RPH3-T1) | PH-3 | PH-3-owned store, separate from runtime DB (DEP-RPH3 §2/§3/§3.1, ODI-RPH3-01, operator-adopted ordered per-domain model); each SHA-pinned + single-transaction; per-domain sole writer incl. operation-intent tables (XSC-RPH3); CMP-WATCH observer remains read-only and the WIR writer is private; R1 preserved |
