...
- **RISK-REC-02** · non-idempotent recovery duplicates changes on replay. Affected: journal, Orchestrator. L Med / I High. Detect: journal-replay (#17), crash (#16). Prevent: `01M §2.18` idempotent ops. Mitigate: replay-safe transitions. Contingency: fail closed to BLOCKED. Owner: PH-2. Threshold: blocks PH-2. Evidence: replay-idempotency PASS. **PH-2 failure plan:** mapped by `docs/planning/PH2-FAILURE-RECOVERY-ROLLBACK.md` F-PH2-01/02/04/09 (detection → `QUARANTINED`/`BLOCKED`; injection INJ-PH2-01/03).
...
- **RISK-MIG-PH2** · a PH-2 runtime migration applies partially or records a version without a successful transactional apply, corrupting the schema baseline. Affected: CMP-ORCH, runtime-state DB. L Low / I High. Detect: migration-runner test (T-PH2-MIG1, REGR-0003), SHA-mismatch test (SEC-PH2-03). Prevent: SHA-256-pinned + one-transaction runner (`01O §2.19`, PH-1 pattern). Mitigate: transactional rollback (RB-PH2-MIG) leaves no partial schema. Contingency: fail closed; re-run verified migration. Owner: PH-2. Threshold: blocks PH-2 (migrations must apply cleanly). Evidence: `docs/planning/PH2-DEPLOYMENT-MIGRATION.md §2/§5`; T-PH2-MIG1 PASS.
