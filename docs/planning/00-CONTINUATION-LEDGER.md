...
| Active pass | Pass 7 (Failure, Recovery, Rollback & Resilience Planning) |
...
| PH-2 Pass 7 | Failure/recovery/rollback/resilience plan (13 failure modes, checkpoints, rollback, recovery, injections, traceability) + Regression Register established | (recorded at pass end) | `docs/planning/PH2-FAILURE-RECOVERY-ROLLBACK.md`, `docs/planning/REGRESSION-REGISTER.md` |

## 7. Next allowed action

Pass 7 produced the PH-2 failure/recovery/rollback/resilience plan (FRR-PH2) and established the Regression
Register (REGR-000, seeded, no repairs recorded) and stops for `CONTINUE`. The Pass 8 specification has not
yet been received. Await the next `SECTION N OF 10` spec and its own `CONTINUE` before beginning the next
pass. Do not begin PH-2 product implementation (operator-gated). Later-phase failure/recovery detail remains
deferred to their phase-planning passes.
